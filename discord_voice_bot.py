...
# ==============================================================================
# 1. KONFIGURASI ENGINE & VOICE SYNTHESIS (EDGE-TTS + APPLIO RVC ZETA)
# ==============================================================================
OWNER_USERNAME = (
    getattr(config, "DISCORD_OWNER_USERNAME", "zakx_x").lower().strip()
)
...
def set_device_mute(muted: bool):
  ...
  if muted:
    print(
        "\n[Mode Suara] 🔕 Speaker Laptop di-MUTE. Tuzi aktif di Voice Channel"
        " Discord."
    )
  else:
    print(
        "\n[Mode Suara] 🔔 Speaker Laptop di-UNMUTE. Tuzi bersuara di laptop"
        " kembali."
    )


def generate_zeta_voice_sync(
    text: str, filename: str = "temp_discord_voice.wav"
) -> bool:
  clean = re.sub(r"\[.*?\]", "", text)
  ...
  if re.search(r"[\u3040-\u30ff\u3400-\u4dbf\u4e00-\u9fff]", clean):
    selected_voice = getattr(
        config, "DEFAULT_EDGE_VOICE_JA", "ja-JP-NanamiNeural"
    )
  elif any(
      w in clean.lower().split()
      for w in [
          "what",
          "the",
          "fuck",
    ...
    selected_voice = getattr(
        config, "DEFAULT_EDGE_VOICE_EN", "en-US-AnaNeural"
    )
  else:
    selected_voice = getattr(
        config, "DEFAULT_EDGE_VOICE_ID", "id-ID-GadisNeural"
    )
  ...
  try:
    ...
    # 1. Coba lewat Fast Server (Memory Standby)
    ...
    try:
      ...
      res = requests.post(
          "http://127.0.0.1:5050/infer", json=payload, timeout=5
      )
      if (
          res.status_code == 200
    ...
    # 2. Fallback via CLI jika server belum dibuka
    if not server_success:
      applio_dir = getattr(config, "APPLIO_DIR", r"E:\Applio-main")
      applio_python = getattr(
          config, "APPLIO_PYTHON", os.path.join(applio_dir, "env", "python.exe")
      )
      applio_core = getattr(
          config, "APPLIO_CORE", os.path.join(applio_dir, "core.py")
      )
      model_path = getattr(
          config,
          "RVC_MODEL_PATH",
          os.path.join(applio_dir, "logs", "zetaTest", "zetaTest.pth"),
      )
      index_path = getattr(
          config,
          "RVC_INDEX_PATH",
          os.path.join(applio_dir, "logs", "zetaTest", "zeta.index"),
      )
      ...
      cmd = [
          ...
          "infer",
          "--f0-method",
          getattr(config, "RVC_F0_METHOD", "rmvpe"),
          "--pitch",
          str(getattr(config, "RVC_PITCH", 4)),
          "--index-rate",
          str(getattr(config, "RVC_INDEX_RATE", 1.0)),
          "--protect",
          str(getattr(config, "RVC_PROTECT", 0.15)),
          "--pth-path",
          model_path,
          "--index-path",
          index_path,
    ...
    print(f"[Discord Voice Gen Error] {e}")
    if os.path.exists(temp_edge):
      try:
        os.remove(temp_edge)
      except Exception:
...
# ==============================================================================
# 2. LANGUAGE DETECTOR & INTENT SANITIZER (UNFILTERED LOGIC)
# ==============================================================================
...
def parse_relay_intent(
    clean_text: str, message: discord.Message
) -> tuple[bool, discord.Member, str]:
  ...
  is_relay = any(
      re.search(kw, text_norm, re.IGNORECASE) for kw in relay_keywords
  )

  if is_relay and target_member:
    action_part = re.sub(
        r"^(?:yo+\s+)?(?:tell|say to|suruh|bilang(?:\s+ke)?)\s+",
        "",
        text_norm,
        flags=re.IGNORECASE,
    )
    ...
    if target_member.display_name:
      action_part = re.sub(
          rf"\b{re.escape(target_member.display_name)}\b",
          "",
          action_part,
          flags=re.IGNORECASE,
      )
    action_part = re.sub(
        r"^(?:this\s+)?(?:guy|bih|bitch|tard|retard|dude|kid|man)?\s*(?:to\s+|that\s+|buat\s+|agar\s+|supaya\s+|kalau\s+)?",
        "",
        action_part.strip(),
        flags=re.IGNORECASE,
    )
...
def sanitize_reply_output(text: str) -> str:
  ...
  cleaned = re.sub(
      r"^(?:<@!?\d+>|@\w+|@\d+)\s*", "", cleaned, flags=re.IGNORECASE
  )
  cleaned = re.sub(
      r"^(?:yo\s+)?tell\s+(?:this\s+)?(?:guy|bih|bitch|retard|tard|dude)?\s*(?:to\s+)?",
      "",
      cleaned,
      flags=re.IGNORECASE,
  ).strip()
  ...
  cleaned = re.sub(
      r"\b(go\s+fuck|fuck)\s+myself\b", r"\1 yourself", cleaned, flags=re.IGNORECASE
  )
  cleaned = re.sub(
      r"\b(shut|stfu)\s+myself\b", r"\1 yourself", cleaned, flags=re.IGNORECASE
  )
...
# ==============================================================================
# 3. AUDIO PLAYBACK ENGINE (HIGH-FIDELITY 48kHZ STEREO STREAM)
# ==============================================================================
async def play_audio_to_vc_async(wav_path: str):
  ...
  def after_done(error):
    if error:
      print(f"[Discord Play Error] {error}")
  ...
  ffmpeg_options = {
      "options": (
          '-vn -filter:a "volume=1.2,aresample=48000:first_pts=0" -ac 2 -ar'
          " 48000"
      )
  }

  audio_source = discord.FFmpegPCMAudio(
      wav_path, executable=FFMPEG_PATH, **ffmpeg_options
  )
...
def play_text_to_vc_sync(text: str):
  ...
  if generate_zeta_voice_sync(text, wav_path):
    asyncio.run_coroutine_threadsafe(
        play_audio_to_vc_async(wav_path), bot_event_loop
    )


# ==============================================================================
# 4. VOICE SINK (REAL-TIME VC LISTENING)
# ==============================================================================
class CustomDiscordVoiceSink(AudioSink):
  ...
  def __init__(self, loop, on_speech_callback):
    ...
    self.monitor_thread = threading.Thread(
        target=self._monitor_silence_loop, daemon=True
    )
  ...
  def _transcribe_and_dispatch(self, user, raw_pcm):
    try:
      ...
      if text and text.strip():
        print(f'\n[Discord VC Heard] 🎙️ {user.display_name}: "{text.strip()}"')
        if self.on_speech_callback and self.loop:
          asyncio.run_coroutine_threadsafe(
              self.on_speech_callback(user, text.strip()), self.loop
          )
...
async def process_and_speak_vc(user, user_text: str):
  ...
  if any(
      k in lower_text
      for k in ["bicara di device", "bicara di laptop", "unmute device"]
  ):
    set_device_mute(False)
    reply_msg = "Baik, aku sekarang bersuara di laptop juga ya."
    print(f'[Discord VC Reply] 🤖 Tuzi: "{reply_msg}"')
  ...
  if any(
      k in lower_text
      for k in [
          "bicara di discord saja",
          "mute device",
          "ngomong di discord aja",
      ]
  ):
...
3. DILARANG memakai emoji."""
  ...
  try:
    ...
    print(f'[Discord VC Reply] 🤖 Tuzi: "{clean_reply}"')

    wav_path = f"vc_reply_{int(time.time())}.wav"
    success = await asyncio.to_thread(
        generate_zeta_voice_sync, clean_reply, wav_path
    )
    ...
    print(f"[Discord VC Process Error] {e}")
  finally:
    is_bot_speaking = False


# ==============================================================================
# 5. DISCORD TEXT CHAT EVENTS & RELAYS
# ==============================================================================
@bot.event
async def on_ready():
  global bot_event_loop
  ...
  print("\n" + "=" * 55)
  print(f"[Discord Bot] 🤖 Bot ONLINE sebagai: {bot.user}")
  print("[Discord Bot] ⚡ Applio Direct RVC Zeta Engine (+14Hz, Pitch +4) Aktif!")
  print("=" * 55 + "\n")
...
async def on_message(message: discord.Message):
  ...
  is_tagged = bot.user.mentioned_in(message) or (
      message.reference
      and message.reference.resolved
      and getattr(message.reference.resolved, "author", None) == bot.user
  )

  if is_tagged and not message.content.startswith(
      getattr(config, "DISCORD_COMMAND_PREFIX", "!")
  ):
    clean_text = message.clean_content.replace(
    ...
    print(
        f"\n[Discord Chat Mention] 📩 {message.author.display_name} di"
        f' #{message.channel.name}: "{clean_text}"'
    )

    lang = detect_language(clean_text)
    is_relay, target_member, action_text = parse_relay_intent(
        clean_text, message
    )
...
3. Bahasa: Indonesia gaul kasar/toxic."""
      ...
      async with message.channel.typing():
        ...
        print(f'[Discord Chat Sent] 🤖 Tuzi: "{final_reply}"')
...
Balas langsung dalam bahasa Indonesia secara to the point tanpa emoji."""
      ...
      async with message.channel.typing():
        ...
        print(f'[Discord Chat Sent] 🤖 Tuzi: "{reply_body}"')


# ==============================================================================
# 6. VC MANAGEMENT & BOT START
# ==============================================================================
...
async def async_trigger_join_vc() -> tuple[bool, str, str]:
  ...
  for guild in bot.guilds:
    for vc in guild.voice_channels:
      for member in vc.members:
        ...
        if (
            OWNER_USERNAME in [m_username, m_display, m_global]
            or OWNER_USERNAME in m_username
        ):
          target_channel = vc
          target_guild = guild
  ...
  if not target_channel:
    return (
        False,
        "OWNER_NOT_IN_VC",
        f"User {OWNER_USERNAME} tidak ada di Voice Channel.",
    )
  ...
  try:
    ...
    set_device_mute(True)
    print(
        f"[Discord Bot] 🚀 Berhasil masuk ke VC '{target_channel.name}' bersama"
        f" '{owner_found_name}'!"
    )
...
def trigger_join_from_voice() -> tuple[bool, str, str]:
  ...
  future = asyncio.run_coroutine_threadsafe(
      async_trigger_join_vc(), bot_event_loop
  )
  try:
...
def trigger_leave_from_voice() -> bool:
  if not bot.is_ready():
    return False
  future = asyncio.run_coroutine_threadsafe(
      async_trigger_leave_vc(), bot_event_loop
  )
  try:
    return future.result(timeout=10)
  except Exception:
...
async def join_cmd(ctx):
  ...
  if await asyncio.to_thread(
      generate_zeta_voice_sync, greeting, "discord_greeting.wav"
  ):
...
def start_discord_bot_thread():
  ...
  def _run():
    ...
    try:
      ...
      print(f"[Discord Bot Error] Gagal menjalankan bot: {e}")
